...
module aes_soc_top_dual_sd_loopback (
    ...
    wire tx_sd_sel   = tx_mem_addr >= SD_BASE && tx_mem_addr < SD_END;
    ...
    wire tx_sd_grant = tx_sd_sel && !sd_owner_is_rx;
    wire rx_sd_grant = rx_sd_sel && sd_owner_is_rx;
    wire shared_sd_valid = tx_sd_grant ? tx_mem_valid : (rx_sd_grant ? rx_mem_valid : 1'b0);
    wire [31:0] shared_sd_addr = tx_sd_grant ? tx_mem_addr : rx_mem_addr;
    wire [31:0] shared_sd_wdata = tx_sd_grant ? tx_mem_wdata : rx_mem_wdata;
    wire [3:0]  shared_sd_wstrb = tx_sd_grant ? tx_mem_wstrb : rx_mem_wstrb;

    assign tx_mem_ready = tx_bram_sel ? tx_bram_ready :
                          (tx_mb_sel ? tx_mb_ready :
                          (tx_sd_grant ? sd_mmio_ready : 1'b1));
    assign tx_mem_rdata = tx_bram_sel ? tx_bram_rdata :
                          (tx_mb_sel ? tx_mb_rdata :
                          (tx_sd_grant ? sd_mmio_rdata : 32'hDEAD_BEEF));

    assign rx_mem_ready = rx_bram_sel ? rx_bram_ready :
                          (rx_rxbuf_sel ? rxbuf_ready :
                          (rx_mb_sel ? rx_mb_ready :
                          (rx_sd_grant ? sd_mmio_ready : 1'b1)));
    assign rx_mem_rdata = rx_bram_sel ? rx_bram_rdata :
                          (rx_rxbuf_sel ? rxbuf_rdata :
                          (rx_mb_sel ? rx_mb_rdata :
                          (rx_sd_grant ? sd_mmio_rdata : 32'hDEAD_BEEF)));
    ...
    bram_memory #(
        ...
        .MEM_INIT_FILE  ("program_tx_loopback.hex")
    ) tx_bram_i (
        .clk       (CLK100MHZ),
        .resetn    (resetn),
    ...
    bram_memory #(
        .MEM_SIZE_WORDS (MEM_SIZE_WORDS),
        .MEM_INIT_FILE  ("program_rx_loopback.hex")
    ) rx_bram_i (
        .clk       (CLK100MHZ),
        .resetn    (resetn),
    ...
    dual_soc_mailbox mailbox_i (
        ...
        .sd_owner_is_rx(sd_owner_is_rx)
    );

    sd_phase1_mmio sd_mmio_i (
        .clk         (CLK100MHZ),
        .resetn      (resetn),
        .mem_valid   (shared_sd_valid),
        .mem_ready   (sd_mmio_ready),
        .mem_addr    (shared_sd_addr),
        .mem_wdata   (shared_sd_wdata),
        .mem_wstrb   (shared_sd_wstrb),
    ...
    assign LED[12] = sd_owner_is_rx;
    assign LED[13] = tx_spi_cs_n;
    assign LED[14] = ~tx_spi_cs_n;
