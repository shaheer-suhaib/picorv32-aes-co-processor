 ...
 *   bytes 0..15  : ciphertext (128 bits)
 *   bytes 16..23 : authentication tag (64 bits, leftmost SHA bits)
 *
 * Memory Map:
 *   BASE + 0x00: RX_STATUS   (R)   - Bit 0: data ready
 *   BASE + 0x04: RX_DATA_0   (R)   - Ciphertext [31:0]
 *   BASE + 0x08: RX_DATA_1   (R)   - Ciphertext [63:32]
 *   BASE + 0x0C: RX_DATA_2   (R)   - Ciphertext [95:64]
 *   BASE + 0x10: RX_DATA_3   (R)   - Ciphertext [127:96]
 *   BASE + 0x14: RX_CLEAR    (W)   - Write any value to clear status
 *   BASE + 0x18: IRQ_ENABLE  (R/W) - Bit 0: enable RX interrupt
 *   BASE + 0x1C: RX_TAG_0    (R)   - Tag [31:0]
 *   BASE + 0x20: RX_TAG_1    (R)   - Tag [63:32]
 */
...
module spi_rx_buffer #(
    ...
    input  wire [191:0] spi_rx_data,   // 16-byte ciphertext + 8-byte tag
    ...
    localparam ADDR_RX_TAG_0    = BASE_ADDR + 32'h1C;
    localparam ADDR_RX_TAG_1    = BASE_ADDR + 32'h20;

    // Check if address is within our range
    wire addr_valid = (mem_addr >= BASE_ADDR) && (mem_addr < BASE_ADDR + 32'h24);
    ...
    reg [127:0] rx_data_buffer;     // Latched ciphertext
    reg [63:0]  rx_tag_buffer;      // Latched authentication tag
    ...
    always @(posedge clk or negedge resetn) begin
        if (!resetn) begin
            ...
            // Capture new packet when valid pulse arrives
            if (spi_rx_valid) begin
                rx_data_buffer <= spi_rx_data[127:0];
                rx_tag_buffer  <= spi_rx_data[191:128];
                rx_data_ready  <= 1'b1;
            end
            ...
            if (mem_valid && addr_valid && !mem_ready) begin
                ...
                if (is_read) begin
                    case (mem_addr)
                        ...
                        ADDR_RX_TAG_0:   mem_rdata <= rx_tag_buffer[31:0];
                        ADDR_RX_TAG_1:   mem_rdata <= rx_tag_buffer[63:32];
